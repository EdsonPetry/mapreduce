...
(cd .. && go build $RACE ./cmd/mrcoordinator/) || exit 1
(cd .. && go build $RACE ./cmd/mrsequential/) || exit 1
(cd .. && go build $RACE ./cmd/mrworker/) || exit 1
...
# start multiple workers.
(
  maybe_quiet $TIMEOUT ../mrworker ../mrapps/early_exit.so
  touch $DF
) &
(
  maybe_quiet $TIMEOUT ../mrworker ../mrapps/early_exit.so
